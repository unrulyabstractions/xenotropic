...
StepFunction = Callable[[torch.Tensor, ModelWrapper], Optional[torch.Tensor]]
...
class Runner:
    ...
    def run_generation(
        ...
        Args:
            ...
            run_step: Function (logits, model) -> next_id (or None to stop)
        ...
        while True:
            ...
            # Call custom step function
            next_id = run_step(logits, self.model)
            ...
            # Check for EOS
            if (self.model.eos_token_id is not None and
                next_id.item() == self.model.eos_token_id):
                break
